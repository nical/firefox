...
use crate::renderer::init::{RenderBackendHooks, SceneBuilderHooks};
...
pub struct PoolMemberSetup {
    ...
    /// Optional callback hooks for the scene builder thread.
...
impl RenderBackendPool {
    ...
    {
        assert!(size > 0, "RenderBackendPool must have at least one member");
