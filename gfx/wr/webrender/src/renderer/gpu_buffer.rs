...
// Only exists so that the generic `finalize` (which patches render-task uv rects
// for both the f32 and i32 GPU buffers) type-checks. Render-task uv rects are
// only ever written to the f32 buffer, so this is never actually called.
