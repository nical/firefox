...
pub unsafe extern "C" fn wr_render_backend_pool_new(
    ...
    let pool = match RenderBackendPool::new(size, move |idx| PoolMemberSetup {
        ...
        // Per-window scene-builder hooks (APZ etc.) are not installed on a
        // shared scene builder; a future step will multiplex them by
        // document. Pool-of-N callers should treat this as a known
        // limitation until the multiplexer lands.
